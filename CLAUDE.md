...
- No devtools, no portal/teleport (overlays are siblings under a `display:contents` wrapper, not in a true root portal), no SSR.
- `Heading` (and the other typography widgets) render a styled `<span>`, **not** a semantic `<h1>`–`<h6>`. There are no ARIA roles, so screen-reader landmarks/heading navigation and SEO heading structure are absent. Surfaced by the e2e suite (`getByRole('heading')` finds nothing); revisit if accessibility becomes a requirement.
